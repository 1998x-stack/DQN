...
if __name__ == '__main__':
    ...
    if isinstance(observation_space, int):
        model = DQN(observation_space, action_space, config.hidden_size_list, dueling=config.dueling).to(config.DEVICE)
        if config.target:
            target_model = DQN(observation_space, action_space, config.hidden_size_list, dueling=config.dueling).to(config.DEVICE)
            target_model.load_state_dict(model.state_dict())
            target_model.eval()
    elif isinstance(observation_space, list):
        model = DQN_CNN(observation_space, action_space, config.channel_kernel_stride_list, dueling=config.dueling).to(config.DEVICE)
        if config.target:
            target_model = DQN_CNN(observation_space, action_space, config.channel_kernel_stride_list, dueling=config.dueling).to(config.DEVICE)
            target_model.load_state_dict(model.state_dict())
            target_model.eval()
    ...
    cumulative_rewards = train_dqn(agent, env, config.episodes)
